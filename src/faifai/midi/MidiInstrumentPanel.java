...
public class MidiInstrumentPanel extends javax.swing.JPanel {

  private Synthesizer synth;
  private MidiChannel channels[];
  private Soundbank soundBank;
  Instrument instrumentArray[];
  private int midiNumberStart = 21;
  private int midiNumberEnd = 108;
  ...
  private void initSelectionLists() {
    Vector instrNameList = new Vector();
    for (int i = 0; i < instrumentArray.length; i++) {
      //if (instrumentArray[i].toString().startsWith("Instrument"))
      instrNameList.add(instrumentArray[i].getName());
    }
    jList2.setListData(instrNameList);
  ...
  public void play() {
    int midiNumber[] = jList1.getSelectedIndices();
    int instruments[] = jList2.getSelectedIndices();
    if (midiNumber.length == 0 || instruments.length == 0) {
      JOptionPane.showMessageDialog(this, "Select one musical note and one instrument", "Midi Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
    for (int i = 0; i < instruments.length; i++) {
      Instrument theInstrument = instrumentArray[instruments[i]];
      Patch thePatch = theInstrument.getPatch();
      channels[1].programChange(thePatch.getBank(), thePatch.getProgram());
      for (int note = 0; note < midiNumber.length; note++) {
        channels[1].noteOn(midiNumber[note], pressure);
      }
    }
  }
  ...
  private void initComponents() {
    ...
    setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Midi Instrument Player", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 3, 11), new java.awt.Color(255, 255, 255))); // NOI18N
